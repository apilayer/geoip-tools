...
// const url = 'https://geoip.tools/v1'
const url = 'http://localhost:3000';

describe('GEOIP TEST JSON', function () {
  ...
  it('JSON /json?q=208.67.222.222 valid ip', function (done) {
    chai.request(url)
      ...
      .end(function (err, res) {
        ...
        expect(res.body).to.have.property('region_code', 'CA');
        ...
        expect(res.body).to.have.property('time_zone', 'America/Los_Angeles');
        expect(res.body).to.have.property('latitude', '37.7697');
        expect(res.body).to.have.property('longitude', '-122.3933');
        done();
      });
  });
...
describe('GEOIP TEST XML', function () {
  before(function (done) {
    done();
  });
  ...
  it('XML /xml?q=github.com valid hostname ', function (done) {
    chai.request(url)
      ...
      .end(function (err, res) {
        ...
        expect(res).to.have.status(200);
        expect(res.text).xml.to.be.valid();
        if (res.text.indexOf('192.30.253.113') === -1) {
...
const text = {
  ...
  'xml': [
    ...
    '<myGeoData><ip>208.67.222.222</ip><country_code>US</country_code><country_name>United States</country_name><region_code>CA</region_code><region_name>California</region_name><city>San Francisco</city><zip_code>94107</zip_code><time_zone>America/Los_Angeles</time_zone><latitude>37.7697</latitude><longitude>-122.3933</longitude></myGeoData>',

    '<myGeoData><ip>8.8.8.8</ip><country_code>US</country_code><country_name>United States</country_name><region_code/><region_name/><city/><zip_code/><time_zone/><latitude>37.7510</latitude><longitude>-97.8220</longitude></myGeoData>',
    ...
    '<myGeoData><ip>192.30.253.112</ip><country_code>US</country_code><country_name>United States</country_name><region_code>CA</region_code><region_name>California</region_name><city>San Francisco</city><zip_code>94107</zip_code><time_zone>America/Los_Angeles</time_zone><latitude>37.7697</latitude><longitude>-122.3933</longitude></myGeoData>',

    '<myGeoData><ip>192.30.253.113</ip><country_code>US</country_code><country_name>United States</country_name><region_code>CA</region_code><region_name>California</region_name><city>San Francisco</city><zip_code>94107</zip_code><time_zone>America/Los_Angeles</time_zone><latitude>37.7697</latitude><longitude>-122.3933</longitude></myGeoData>',
